...
const Preloader = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      setTimeout(() => setLoading(false), 500); // delay hilang 0.5s
    }, 2000); // preload 2 detik
  ...
  return (
    <div
      className={`w-screen h-screen fixed top-0 left-0 flex flex-col items-center justify-center bg-black z-50 transition-opacity duration-500 ${
    ...
    >
      ...
      <div role="status">
        <svg
          aria-hidden="true"
          className="w-8 h-8 sm:w-10 sm:h-10 text-gray-200 animate-spin fill-purple-600"
          viewBox="0 0 100 101"
